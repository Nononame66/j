 ...
 * Represents the energy balls that the player must rescue
 */
class Ball {
  constructor(x, y, vx = 0, vy = 0, type = 'normal') {
    this.x = x;
    this.y = y;
    this.vx = vx !== 0 ? vx : (Math.random() - 0.5) * GAME_CONFIG.BALL.INITIAL_VX_RANGE;
    this.vy = vy !== 0 ? vy : GAME_CONFIG.BALL.INITIAL_VY_MIN - Math.random() * (GAME_CONFIG.BALL.INITIAL_VY_MAX - GAME_CONFIG.BALL.INITIAL_VY_MIN);
    this.radius = GAME_CONFIG.BALL.RADIUS;
    this.type = type; // 'normal', 'gold', 'ice', 'plasma'
    
    this.trail = [];
    this.maxTrail = GAME_CONFIG.BALL.MAX_TRAIL_LENGTH;
    this.speed = Math.hypot(this.vx, this.vy);

    // Color maps for different ball types - Soft Pink Theme
    this.colorMap = {
      normal: { core: '#ffffff', glow: '#f8bbd0', main: '#f48fb1' },
      gold: { core: '#ffffff', glow: '#ffd700', main: '#ffaa00' },
      ice: { core: '#ffffff', glow: '#e1bee7', main: '#ce93d8' },
      plasma: { core: '#ffffff', glow: '#f06292', main: '#ec407a' }
    };

    if (this.type === 'gold') {
      this.radius = GAME_CONFIG.BALL.GOLD_RADIUS;
    }
  }
  ...
  update(width, height, particleSystem, slowMotionFactor = 1) {
    try {
      ...
      console.warn('Ball update error:', error);
      return null;
    }
  }
  ...
  checkBatCollision(bat, particleSystem, soundEngine) {
    try {
      ...
      if (
        ...
        const bounceAngle = clampedOffset * GAME_CONFIG.BALL.MAX_BOUNCE_ANGLE;
        const currentSpeed = Math.max(GAME_CONFIG.BALL.MIN_SPEED, Math.hypot(this.vx, this.vy) * GAME_CONFIG.BALL.SPEED_INCREASE_ON_HIT);
      ...
      if (bat.isMagnet) {
        ...
        if (dist < GAME_CONFIG.BAT.MAGNET_RANGE) {
          this.vx += (dx / dist) * 0.6;
          this.vy += (dy / dist) * 0.4;
        }
      }
      ...
      console.warn('Ball collision check error:', error);
      return false;
    }
  }
  ...
  draw(ctx) {
    try {
      ...
      console.warn('Ball render error:', error);
    }
  }
}
